...
public class TrainMalletCRF {
	...
	public void addSequenceInstance(List<String> labels,List<List<String>> stateFeatures){
		if(labels.size()!=stateFeatures.size())
			throw new RuntimeException("not same number of labels and states");
		FeatureVectorSequence featureVectorSequence = stateFeatures2FVS(stateFeatures,featureAlphabet);
		LabelSequence labelSequence = new LabelSequence(this.labelAlphabet);
		for (int i = 0; i < labels.size(); i++)
			labelSequence.add(labels.get(i));
		
		Instance instance = new Instance(featureVectorSequence, labelSequence,	null, null);
		isList.add(instance);
	}
		...
		for (int i = 1; i <= iterations; i++) {
			try {
				...
				if (crfTrainer.train(trainingData, 1)) {
					System.err.println("Finished early after "+i+" iterations");
					break;
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	static AugmentableFeatureVector features2AFV(List<String> features,Alphabet featureAlphabet){
		AugmentableFeatureVector afv=new AugmentableFeatureVector(featureAlphabet, 100, true);
	...
	static FeatureVectorSequence stateFeatures2FVS(List<List<String>> stateFeatures,Alphabet featureAlphabet) {
		AugmentableFeatureVector[] augmentableFeatureVectorArray = new AugmentableFeatureVector[stateFeatures.size()];
		for (int i = 0; i < stateFeatures.size(); i++) {
			augmentableFeatureVectorArray[i] = features2AFV(stateFeatures.get(i),featureAlphabet);
			
		}
		FeatureVectorSequence featureVectorSequence = new FeatureVectorSequence(augmentableFeatureVectorArray);
